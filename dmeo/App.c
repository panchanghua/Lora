...
void print(u8 buffer[5])
{
    ...
    if(dht11_read_data(buffer) == 0)
    {
          ...
          DEBUG_Printf("___{\"temperature\": %.2f, \"humidness\": %.2f}___\n", temp, hum);
    }
    else
      DEBUG_Printf("温度获取失败\r\n");
}
					...
					if(LoRa_Node.Join == 1)
					{
						LED4_ON;
                                                while(1)
                                                {
                                                    ...
                                                    {
                                                        DEBUG_Printf("%d\r\n",Usart1_RX.RX_Buf[0]);
                                                        LoraNode_Write_Receive_Bytes(SS_Data,lppsize,LoRa_Node.Confirm);
...
uint8_t ENV_Sensor_Acquier()
{
  ...
  uint16_t humi16=0;
  uint8_t rspIdx=0;
  //uint8_t sensorRsp[SensorRSP_LEN] = {0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00};
