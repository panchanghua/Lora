...
//	while(HAL_I2C_Master_Transmit(MMA8451_Handle, MMA8451_WRITE_MODE, &addr, 1, 1000) != HAL_OK);		
//
//	while(HAL_I2C_Master_Receive(MMA8451_Handle, MMA8451_READ_MODE, buffer, len, 1000) != HAL_OK);
...
ACCELER_T MMA8451_ReadAcceleration()
{
	uint8_t statu;
	ACCELER_T accel = {0};
	uint8_t msb = 0, lsb = 0;
